...
const TRAVEL_ORDER_STATUS_PENDING = 1;
const TRAVEL_ORDER_STATUS_APPROVED = 2;
const TRAVEL_ORDER_STATUS_REJECTED = 3;
const TRAVEL_ORDER_STATUS_CANCELLED = 4;
...
function getTravelOrderIdFromQuery(): ?int
{
    $rawId = $_GET['id'] ?? null;
    $travelOrderId = filter_var($rawId, FILTER_VALIDATE_INT);

    return $travelOrderId === false ? null : $travelOrderId;
}

function ensureTravelOrdersTableExists(PDO $conn): void
{
    $conn->exec(
...
function travelOrdersTableUsesAutoIncrement(PDO $conn): bool
{
    $statement = $conn->query("SHOW COLUMNS FROM `tbltravel_orders` LIKE 'to_id'");
    $column = $statement->fetch();

    return $column && stripos((string) ($column['Extra'] ?? ''), 'auto_increment') !== false;
}

function getNextTravelOrderId(PDO $conn): int
{
    $statement = $conn->query('SELECT COALESCE(MAX(`to_id`), 0) + 1 AS `next_id` FROM `tbltravel_orders`');
...
function calculateTravelDays(string $startDate, string $endDate): int
{
...
function validateCreatePayload(PDO $conn, array $payload): array
{
    ...
    $assistanceOrLaborAllowed = normalizeOptionalString($payload, 'assistance_or_labor_allowed', 255);
    $appropriations = normalizeOptionalString($payload, 'appropriations', 255);
    $remarks = normalizeOptionalString($payload, 'remarks', 5000);
    ...
    calculateTravelDays($startDate, $endDate);

    return [
        'appropriations' => $appropriations,
        'assistance_or_labor_allowed' => $assistanceOrLaborAllowed,
        ...
        'status' => TRAVEL_ORDER_STATUS_PENDING,
    ];
}
...
function validateEditPayload(PDO $conn, array $payload): array
{
    ...
    return [
        'appropriations' => $validatedPayload['appropriations'],
        'assistance_or_labor_allowed' => $validatedPayload['assistance_or_labor_allowed'],
...
function validateDecisionPayload(array $payload): array
{
    ...
    if (!in_array($status, [TRAVEL_ORDER_STATUS_APPROVED, TRAVEL_ORDER_STATUS_REJECTED, TRAVEL_ORDER_STATUS_CANCELLED], true)) {
        respond(422, [
    ...
    if (
        in_array($status, [TRAVEL_ORDER_STATUS_APPROVED, TRAVEL_ORDER_STATUS_REJECTED], true) &&
...
function fetchTravelOrders(PDO $conn, bool $hasLegacyNameColumn, ?string $employeeEmail = null): array
{
    ...
    $query = "SELECT
            `tbltravel_orders`.`to_id`,
            CONCAT('TO-', LPAD(`tbltravel_orders`.`to_id`, 3, '0')) AS `request_code`,
            `tbltravel_orders`.`employee_id` AS `employee_record_id`,
            ...
            `divisions`.`name` AS `division`,
            `designations`.`name` AS `designation`,
            `tbltravel_orders`.`destination`,
            `tbltravel_orders`.`start_date`,
            `tbltravel_orders`.`end_date`,
            DATEDIFF(`tbltravel_orders`.`end_date`, `tbltravel_orders`.`start_date`) + 1 AS `travel_days`,
            `tbltravel_orders`.`purpose`,
            `tbltravel_orders`.`assistance_or_labor_allowed`,
            `tbltravel_orders`.`appropriations`,
            `tbltravel_orders`.`remarks`,
            `tbltravel_orders`.`status`,
            CASE `tbltravel_orders`.`status`
                WHEN 1 THEN 'Pending'
                ...
                ELSE 'Unknown'
            END AS `status_label`,
            `tbltravel_orders`.`approved_by`,
            `tbltravel_orders`.`approved_at`,
            `tbltravel_orders`.`rejected_by`,
            `tbltravel_orders`.`rejected_at`,
            `tbltravel_orders`.`created_at`,
            {$approverDisplaySelect} AS `approved_by_name`,
            {$rejectorDisplaySelect} AS `rejected_by_name`
         FROM `tbltravel_orders`
         INNER JOIN `employees` ON `employees`.`id` = `tbltravel_orders`.`employee_id`
         INNER JOIN `divisions` ON `divisions`.`id` = `employees`.`division_id`
         INNER JOIN `designations` ON `designations`.`id` = `employees`.`designation_id`
         LEFT JOIN `users` AS `approved_user` ON `approved_user`.`id` = `tbltravel_orders`.`approved_by`
         LEFT JOIN `users` AS `rejected_user` ON `rejected_user`.`id` = `tbltravel_orders`.`rejected_by`";
    $params = [];
    ...
    $query .= ' ORDER BY `tbltravel_orders`.`created_at` DESC, `tbltravel_orders`.`to_id` DESC';
...
function fetchTravelOrderById(PDO $conn, bool $hasLegacyNameColumn, int $travelOrderId): ?array
{
    $travelOrders = fetchTravelOrders($conn, $hasLegacyNameColumn);

    foreach ($travelOrders as $travelOrder) {
        if ((int) $travelOrder['to_id'] === $travelOrderId) {
            return $travelOrder;
        }
    }
...
try {
    ensureTravelOrdersTableExists($conn);
    ...
    switch ($_SERVER['REQUEST_METHOD']) {
        case 'GET':
            ...
            respond(200, [
                ...
                'travelOrders' => fetchTravelOrders($conn, $hasLegacyNameColumn, $employeeEmail ?: null),
            ]);
            break;

        case 'POST':
            $payload = validateCreatePayload($conn, parseJsonPayload());
            $usesAutoIncrement = travelOrdersTableUsesAutoIncrement($conn);
            $insertedTravelOrderId = null;

            if ($usesAutoIncrement) {
                $statement = $conn->prepare(
                    'INSERT INTO `tbltravel_orders` (
                        `employee_id`,
                        `destination`,
                        `start_date`,
                        `end_date`,
                        `purpose`,
                        `assistance_or_labor_allowed`,
                        `appropriations`,
                        `remarks`,
                        `status`
                    ) VALUES (
                ...
                $insertedTravelOrderId = getNextTravelOrderId($conn);
                $statement = $conn->prepare(
                    'INSERT INTO `tbltravel_orders` (
                        `to_id`,
                        `employee_id`,
                ...
                $payload['to_id'] = $insertedTravelOrderId;
            }

            $statement->execute($payload);

            if ($insertedTravelOrderId === null) {
                $insertedTravelOrderId = (int) $conn->lastInsertId();
            }

            respond(201, [
                'message' => 'Travel order submitted successfully.',
                'travelOrder' => fetchTravelOrderById($conn, $hasLegacyNameColumn, $insertedTravelOrderId),
            ]);
            break;

        case 'PUT':
            $travelOrderId = getTravelOrderIdFromQuery();

            if ($travelOrderId === null) {
                respond(422, [
                    'message' => 'A valid travel order id is required.',
                ]);
            }

            $existingTravelOrder = fetchTravelOrderById($conn, $hasLegacyNameColumn, $travelOrderId);

            if ($existingTravelOrder === null) {
                respond(404, [
                    'message' => 'The selected travel order was not found.',
                ]);
            }
            ...
            if (($rawPayload['mode'] ?? '') === 'edit') {
                if ((int) $existingTravelOrder['status'] !== TRAVEL_ORDER_STATUS_PENDING) {
                    respond(422, [
                ...
                $statement = $conn->prepare(
                    'UPDATE `tbltravel_orders`
                     SET
                        ...
                        `end_date` = :end_date,
                        `purpose` = :purpose,
                        `assistance_or_labor_allowed` = :assistance_or_labor_allowed,
                        `appropriations` = :appropriations,
                        `remarks` = :remarks,
                        `status` = :status,
                        `approved_by` = NULL,
                ...
                $statement->execute([
                    'appropriations' => $payload['appropriations'],
                    'assistance_or_labor_allowed' => $payload['assistance_or_labor_allowed'],
                ...
                respond(200, [
                    ...
                    'travelOrder' => fetchTravelOrderById($conn, $hasLegacyNameColumn, $travelOrderId),
                ]);
            }
            ...
            if (
                in_array($payload['status'], [TRAVEL_ORDER_STATUS_APPROVED, TRAVEL_ORDER_STATUS_REJECTED], true) &&
                $payload['admin_user_id'] !== null &&
                isUserTryingToReviewOwnRequest(
                    $conn,
                    $payload['admin_user_id'],
                    $existingTravelOrder['employee_email'] ?? null
                )
            ) {
                respond(403, [
                    'message' => 'You cannot approve or reject your own travel order. Another user must review it.',
                ]);
            }

            $isApproved = $payload['status'] === TRAVEL_ORDER_STATUS_APPROVED;
            $isRejected = $payload['status'] === TRAVEL_ORDER_STATUS_REJECTED;
            $statement = $conn->prepare(
                'UPDATE `tbltravel_orders`
                 SET
                    `status` = :status,
    ...
    respond(500, [
        'message' => 'Unable to process travel orders right now.',
    ]);
}
?>
